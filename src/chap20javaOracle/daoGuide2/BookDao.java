...
public class BookDao implements Dao<Book, Integer> { // int가 아니라 Integer 사용함
	...
	public int insert(Connection conn, Book vo) throws SQLException {
		...
		PreparedStatement pstmt = conn.prepareStatement(sql); // pstst는 conn으로 사용하는 것임
		pstmt.setInt(1, vo.getBookid());
		pstmt.setString(2,vo.getBookname());
		pstmt.setString(3,vo.getPublisher());
		pstmt.setInt(4,vo.getPrice());
		int res = pstmt.executeUpdate();//select 질의는 pstmt.executeQuery() sql이 있는만큼 업데이트할 것
		
		if (conn != null)
			pstmt.close(); // 나중에 사용한 것을 먼저 닫음
		if (conn != null)
			conn.close();
		return res;
	}
