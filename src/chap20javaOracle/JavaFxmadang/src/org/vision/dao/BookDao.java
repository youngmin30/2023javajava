package chap20javaOracle.JavaFxmadang.src.org.vision.dao;
...
public class BookDao implements Dao<Book, Integer> {
	...
	public int insert(Connection conn, Map<String, Object> map) throws SQLException {
		String sql = "INSERT INTO BOOK (BOOKID,BOOKNAME,PUBLISHER,PRICE) VALUES(?,?,?,?)";
		PreparedStatement pstmt = conn.prepareStatement(sql);
		pstmt.setInt(1, (Integer)map.get("BOOKID"));
		pstmt.setString(2,(String)map.get("BOOKNAME"));
		pstmt.setString(3,(String)map.get("PUBLISHER"));
		pstmt.setInt(4,(Integer)map.get("PRICE"));
		int res = pstmt.executeUpdate();//select 질의는 pstmt.executeQuery()
		if (conn != null)
			pstmt.close();
		if (conn != null)
	...
	public List<Book> selectCondition(Connection conn, String condition) throws SQLException {
		String sql = "SELECT * FROM BOOK "+condition;
		PreparedStatement pstmt = conn.prepareStatement(sql);
		ResultSet rs = pstmt.executeQuery();
		List<Book> list = new ArrayList<>();
		while (rs.next()) {
